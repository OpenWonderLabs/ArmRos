...
private:
  ...
  void planAndExecute(const std::vector<double> &target_joints)
  {
    ...
    try
    {
      ...
      RCLCPP_INFO(this->get_logger(), "规划手臂检测: %s", planning_arms_status.c_str());
      ...
      RCLCPP_INFO(this->get_logger(), "需要交换关节顺序: %s", needs_swap ? "是" : "否");//debug
  ...
  void displayPlannedPathCallback(const moveit_msgs::msg::DisplayTrajectory::SharedPtr msg)
  {
    ...
    // DEBUG: 将接收到的轨迹写入文件, 顺序是先右臂，再左臂（moveit_trajectory_record.txt）
    // 暂时关闭轨迹文件记录，避免运行时在工作目录生成 moveit_trajectory_record.txt。
#if 0
    std::ofstream ofs("moveit_trajectory_record.txt", std::ios::trunc); // 使用 trunc 清空旧文件
    if (ofs.is_open()) {
        int dof = dof_; // 使用成员变量 dof_
        for (const auto& point : trajectory.points) {
            // 写入时间戳
            ofs << rclcpp::Duration(point.time_from_start).seconds() << " ";
        ...
        ofs.flush();
        RCLCPP_INFO(this->get_logger(), "MoveIt轨迹已写入 moveit_trajectory_record.txt");
    } else {
        RCLCPP_ERROR(this->get_logger(), "无法打开 moveit_trajectory_record.txt 进行写入");
    }
#endif
