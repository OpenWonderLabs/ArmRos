...
- 将离散后的关节角度和速度通过 `/moveit_angle` 相关执行链路交给 `onero_driver`。
...
| 话题名称                             | 消息类型                              | 频率     | 说明                                           |
| ------------------------------------ | ------------------------------------- | -------- | ---------------------------------------------- |
| `/moveit_trajectory`                 | `trajectory_msgs/msg/JointTrajectory` | 事件触发 | 发布完整规划轨迹，供`onero_driver` 执行         |
| `/moveit_planner_status`             | `std_msgs/msg/String`                 | 事件触发 | 发布规划器状态，例如规划中、成功、失败或执行中 |
| `/moveit_execute`                    | `std_msgs/msg/Empty`                  | 事件触发 | 通知驱动侧开始执行轨迹                         |
| `/moveit_cancel`                     | `std_msgs/msg/Empty`                  | 事件触发 | 通知驱动侧取消当前轨迹                         |
| `/moveit_planner/display_trajectory` | `moveit_msgs/msg/DisplayTrajectory`   | 事件触发 | 发布离散后的轨迹，供 RViz 显示                 |
...
| ------------------------------------ | ------------------------------------- | -------- | ------------------------------------------------------------ |
| `/moveit_trajectory`                 | `trajectory_msgs/msg/JointTrajectory` | 事件触发 | 发布完整规划轨迹，供`onero_driver` 执行                       |
| `/moveit_planner_status`             | `std_msgs/msg/String`                 | 事件触发 | 发布规划器状态信息                                           |
| `/moveit_planning_arms_status`       | `std_msgs/msg/String`                 | 事件触发 | 发布当前规划对象，取值如`left_arm`、`right_arm`、`both_arms` |
| `/moveit_execute`                    | `std_msgs/msg/Empty`                  | 事件触发 | 通知驱动侧开始执行轨迹                                       |
| `/moveit_cancel`                     | `std_msgs/msg/Empty`                  | 事件触发 | 通知驱动侧取消当前轨迹                                       |
| `/moveit_planner/display_trajectory` | `moveit_msgs/msg/DisplayTrajectory`   | 事件触发 | 发布离散后的轨迹，供 RViz 显示                               |
