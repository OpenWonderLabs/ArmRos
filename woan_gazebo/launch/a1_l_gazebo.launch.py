...
def generate_launch_description():
    ...
    robot_name = 'a1_l_robot'

    # 获取Gazebo专用URDF路径
    pkg_share = get_package_share_directory(package_name)
    urdf_file = os.path.join(pkg_share, 'config', 'a1_l_gazebo_description.urdf.xacro')
    
    # 获取woan_description包的路径用于构建file://路径
    woan_description_pkg = get_package_share_directory('woan_description')

    # 解析xacro文件
    print(f"[INFO] 加载URDF文件: {urdf_file}")
    doc = xacro.parse(open(urdf_file))
    # 传递 mesh_prefix 参数给xacro处理 - 使用file://协议加绝对路径避免Gazebo转换问题
    xacro.process_doc(doc, mappings={'mesh_prefix': f'file://{woan_description_pkg}'})
