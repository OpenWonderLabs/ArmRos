...
/**
 * @brief OneroArm机器人驱动节点
 * 
 * 提供ROS2话题接口，接收Move API命令并转发给arm_control执行。
 * 发布机械臂状态信息。
 */
class OneroDriver : public rclcpp::Node {
public:
    OneroDriver();
    ~OneroDriver();
...
private:
    ...
    rclcpp::Publisher<onero_interfaces::msg::ArmState>::SharedPtr arm_state_pub_;
    
    // 转发到arm_control的发布器
    rclcpp::Publisher<onero_interfaces::msg::MoveJ>::SharedPtr arm_movej_pub_;
    rclcpp::Publisher<onero_interfaces::msg::MoveL>::SharedPtr arm_movel_pub_;
    rclcpp::Publisher<onero_interfaces::msg::MoveP>::SharedPtr arm_movep_pub_;
