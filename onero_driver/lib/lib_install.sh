...
case "$RAW_ARCH" in
    ...
    aarch64|arm64)
        ...
        ;;
    riscv64)
        PLATFORM_TAG="linux_riscv64_v1.0.0"
        EXPECTED_MACHINE="RISC-V"
        ;;
    *)
        echo -e "${RED}✗ 错误: 当前架构 $RAW_ARCH 暂不支持${NC}"
        echo -e "${YELLOW}  支持: x86_64, aarch64, riscv64${NC}"
        exit 1
        ;;
esac
...
# 在本地 lib 目录创建指向当前架构 .so 的软链接（避免重复二进制，且始终与源同步）
if [ "$SO_SOURCE" != "./libonero_api.so" ]; then
    ln -sf "$SO_SOURCE" ./libonero_api.so
fi
