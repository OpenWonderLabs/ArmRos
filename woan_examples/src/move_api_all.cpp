...
public:
    MoveApiAllDemo() : Node("move_api_all_demo"), result_received_(false), current_api_("") {
        // 创建发布器
        movej_pub_ = this->create_publisher<woan_interfaces::msg::MoveJ>("/woan_driver/movej_cmd", 10);
        movel_pub_ = this->create_publisher<woan_interfaces::msg::MoveL>("/woan_driver/movel_cmd", 10);
        movep_pub_ = this->create_publisher<woan_interfaces::msg::MoveP>("/woan_driver/movep_cmd", 10);
...
private:
    ...
    void sendMoveJ(const std::vector<float>& joint_angles) {
        auto msg = std::make_shared<woan_interfaces::msg::MoveJ>();
    ...
    void sendMoveL(double x, double y, double z) {
        auto msg = std::make_shared<woan_interfaces::msg::MoveL>();
    ...
    void sendMoveP(double x, double y, double z) {
        auto msg = std::make_shared<woan_interfaces::msg::MoveP>();
        msg->pose.position.x = x;
        msg->pose.position.y = y;
        msg->pose.position.z = z;
        msg->pose.orientation.w = 1.0;
