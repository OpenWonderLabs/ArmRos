...
class StateMonitor: public rclcpp::Node
{
  public:
    ...
    void StateMonitor_Callback(const woan_interfaces::msg::ArmState & msg);                         //结果回调函数
  
  private:
    rclcpp::Publisher<std_msgs::msg::Empty>::SharedPtr publisher_;                                   //声明发布器
    rclcpp::Subscription<woan_interfaces::msg::ArmState>::SharedPtr subscription_;                    //声明订阅器
};


/******************************接收到订阅的机械臂执行状态消息后，会进入消息回调函数**************************/ 
void StateMonitor::StateMonitor_Callback(const woan_interfaces::msg::ArmState & msg)
{
...
{
  subscription_ = this->create_subscription<woan_interfaces::msg::ArmState>("/woan_driver/arm_state", rclcpp::ParametersQoS(), std::bind(&StateMonitor::StateMonitor_Callback, this,_1));
  publisher_ = this->create_publisher<std_msgs::msg::Empty>("/woan_driver/get_current_arm_state_cmd", rclcpp::ParametersQoS());
