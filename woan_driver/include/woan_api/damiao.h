...
#define Motor_id uint32_t
    ...
    // 移动至slcan.h DamiaoSlcan类内，不再暴露给motor_control类
    // typedef struct
    // {
    //     uint8_t FrameHeader;
 ...
 */
    ...
    public:
        ...
        using SharedPtr = std::shared_ptr<Motor_Control>;
        /*
        * @brief 定义电机控制对象
        * @param serial      串口对象（可选）
        * @param slcan_type  协议类型："canable" | "damiao"
        */
        Motor_Control(
            SerialPort::SharedPtr serial = nullptr,
            const std::string& slcan_type = "damiao"
        )
        {
            // 1️⃣ 初始化串口
            if (serial == nullptr) {
                serial_ = std::make_shared<SerialPort>("/dev/ttyACM0", 921600);
            } else {
            ...
            if (slcan_type == "canable") {
                slcan_ = std::make_shared<CanableSlcan>(serial_);
            }
            else if (slcan_type == "damiao") {
                slcan_ = std::make_shared<DamiaoSlcan>(serial_);
            }
            else {
                throw std::runtime_error("Unknown SLCAN type: " + slcan_type);
            }
        ...
        */
        ...
        void refresh_motor_status(const Motor& motor)
        {
            ...
            slcan_->send(id, std::vector<uint8_t>(data_buf.begin(), data_buf.end()));
            this->receive();
        }
        /*
        ...
        */
        ...
        void receive()
        {
            // 这里的局部变量替代了原有的 CAN_Receive_Frame receive_data
            uint16_t frame_id = 0;
            std::vector<uint8_t> data;

            // 1. 尝试接收一帧标准 CAN 数据
            // 此时不需要关心是 Canable 还是 Damiao，也不需要关心 CMD 是 0x11 还是 0xEE
            // 只要 recv 返回 true，就说明收到了一帧针对某个 ID 的 8 字节数据
            if (!slcan_->recv(frame_id, data))
            {
                return; 
            }

            if (data.size() < 8) return; // 简单校验
            ...
            // --- Gripper ---
            if(gripper != nullptr && frame_id == gripper->GetNodeId())
            {
                gripper->receive_data(data[0], data[1], data[2], data[3], data[4]);
                return;
            }
            // --- Joystick ---
            else if(gripper_joystick != nullptr && frame_id == gripper_joystick->GetNodeId())
            {
                uint16_t joy = (uint16_t(data[1]) << 8) | data[2];
                gripper_joystick->receive_data(data[3], data[4], data[5], data[6], data[0], joy);
                return;
            }

            // --- Motors ---
            // 这里不再需要判断 CMD == 0x11 或 0xEE，因为 Slcan::recv 已经过滤了其他无关指令
            
            // 解析公共数据 (根据达妙CAN帧格式和表格)
            // Byte0: [ERR(4位) | ID(4位)]
            // Byte1: POS[15:8], Byte2: POS[7:0] → 16位位置
            // Byte3: VEL[11:4], Byte4高4位: VEL[3:0] → 12位速度
            // Byte4低4位: T[11:8], Byte5: T[7:0] → 12位力矩
            ...
            // 调试输出
            // std::cout << "Frame ID: 0x" << std::hex << frame_id 
            //           << " q_uint: " << std::dec << q_uint 
            //           << " dq_uint: " << dq_uint 
            //           << " tau_uint: " << tau_uint << std::endl;
            ...
            if (frame_id != 0x00)
            {
                if(motors.find(frame_id) != motors.end())
                    m = motors[frame_id];
                else
                    std::cout << "Warning: Motor with ID 0x" << std::hex << frame_id << " not found in motors map!" << std::dec << std::endl;
            }
            else // 处理 frame_id 为 0 的特殊情况 (MasterID=0)
            {
                uint32_t slaveID = data[0] & 0x0f; // 从数据中获取真实ID
            ...
            if (m != nullptr)
            {
                ...
                // 如果 err_nibble 不为0，说明有错误（对应原 CMD=0xEE 或 CMD=0x11带错误码的情况）
                // 原逻辑中 CMD=0xEE 会打印错误，这里可以选择性添加：
                if (err_nibble != 0) {
                // std::cerr << "Motor Error: " << Motor::error_code_to_string(err_nibble) << std::endl;
                }
        ...
        void receive_param()
        {
            ...
            // 1. 调用适配层的 recv 接口
            // DamiaoSlcan::recv 内部已经完成了以下工作：
            // - 校验帧头 0xAA 和 帧尾 0x55
            // - 校验 CMD (通常只放行 0x11 和 0xEE)
            // - 提取 8 字节数据到 data
            if (!slcan_->recv(frame_id, data)) 
            {
                return; 
            ...
            // 3. 业务逻辑判断
            // 原代码逻辑：检查 data[2] 是否为参数读/写返回标志 (0x33 或 0x55)
            // 注意：原来的 receive_data.canData[i] 现在直接变成了 data[i]
            if(data[2] == 0x33 || data[2] == 0x55)
            {
                // 4. 解析 SlaveID (小端模式：data[0]是低位，data[1]是高位)
                uint32_t slaveID = (uint32_t(data[1]) << 8) | data[0];
                
                // 5. 查找电机对象
                // 注意：motors 是 map<Motor_id, Motor*>
                auto it = motors.find(slaveID);
                if (it == motors.end())
                {
                    // can not found motor id
                    return;
                }
                Motor* m = it->second; // 获取对应的电机指针

                // 6. 解析参数 ID (RID)
                uint8_t RID = data[3];

                // 7. 根据参数类型解析数值 (Int 或 Float)
                if(is_in_ranges(RID))
                {
                    // 解析 uint32_t (小端模式：data[4]~data[7])
                ...
                {
                    // 解析 float
                    // 原代码: uint8_to_float(data + 4); 这里的 data 是数组首地址
                    // 新代码: data 是 vector，获取内部指针需要使用 data.data()
                    // 确保 uint8_to_float 函数在当前类中可访问
                    float data_float = uint8_to_float(data.data() + 4);
                    
                    m->set_param(RID, data_float);
                }
            }
            // 原代码最后的 return; 是多余的，函数结束自动返回
        }
        ...
        void control_gripper(const Gripper* gripper, uint8_t pos, uint8_t vel, uint8_t force)
        {
            ...
            slcan_->send(gripper->GetNodeId(), std::vector<uint8_t>(data_buf.begin(), data_buf.end()));
            this->receive();
        }
        ...
        void control_gripper_joystick(const GripperJoystick* gripper_joystick, uint8_t led)
        {
            ...
            slcan_->send(gripper_joystick->GetNodeId(), std::vector<uint8_t>(data_buf.begin(), data_buf.end()));
            this->receive();
        }
        ...
        float read_motor_param(Motor &DM_Motor,uint8_t RID)
        {
            ...
            {
                std::this_thread::sleep_for(std::chrono::microseconds(retry_interval));
                receive_param();
    ...
    private:
        ...
        std::shared_ptr<SlcanProtocolBase> slcan_; //slcan protocol
    };
