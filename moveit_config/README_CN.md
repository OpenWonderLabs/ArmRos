...
`onero_driver_node` 还会订阅 `/onero_control_node` 发布的 `/onero_arm/moveit/trajectory`。该话题承载离散后的关节轨迹，`onero_driver` 依据这些数据控制机械臂执行规划轨迹。

对应关系如下图所示。
![image](doc/onero_moveit2_config6.png)

`onero_control_node` 本质上承担了 MoveIt2 到底层驱动之间的桥接任务。它通过 `/arm_controller/follow_joint_trajectory` 动作接口与 `/moveit_simple_controller_manager` 协同，获取规划轨迹后进行插值，再通过 `/onero_arm/moveit/trajectory`、`/onero_arm/moveit/execute`、`/onero_arm/moveit/cancel` 等桥接话题透传给驱动节点。
