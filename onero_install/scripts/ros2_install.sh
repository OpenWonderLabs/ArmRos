...
install_optional_package() {
    ...
    if ! apt_has_candidate "$package"; then
        echo "Skipping optional package $package: no APT candidate for this OS/architecture."
        return 1
    fi

    if ! sudo apt-get install -s "$package" >/dev/null 2>&1; then
        echo "Skipping optional package $package: dependencies cannot be resolved."
        return 1
    fi
...
install_optional_packages() {
    local package
    for package in "$@"; do
        install_optional_package "$package" || true
    done
}
...
sudo cp /etc/apt/sources.list /etc/apt/sources.list.d/oneroarm_ros2.list
# Clear original software sources
sudo echo "" > /etc/apt/sources.list
sudo echo "" > /etc/apt/sources.list.d/oneroarm_ros2.list
...
echo "deb [arch=$(dpkg --print-architecture)] https://mirrors.tuna.tsinghua.edu.cn/ros2/ubuntu $(. /etc/os-release && echo $UBUNTU_CODENAME) main" >> /etc/apt/sources.list.d/oneroarm_ros2.list
...
echo "deb [arch=$(dpkg --print-architecture)] https://mirrors.tuna.tsinghua.edu.cn/ros2/ubuntu jammy main" >> /etc/apt/sources.list.d/oneroarm_ros2.list
if [ $(uname -m) = "x86_64" ]; then
...
# Refresh package index after replacing APT sources
sudo apt-get update
...
if [ $Ubuntu_version = "20.04" ]; then
  ...
  if install_optional_package gazebo11; then
    install_optional_packages \
      ros-foxy-gazebo-ros-pkgs \
      ros-foxy-gazebo-dev \
      ros-foxy-gazebo-msgs
  else
    echo "Skipping ROS Gazebo packages because Gazebo is unavailable."
  fi
  ROS_DISTRO="foxy"
elif [ $Ubuntu_version = "22.04" ]; then
  sudo apt-get install ros-humble-desktop -y
  if install_optional_package gazebo; then
    install_optional_packages \
