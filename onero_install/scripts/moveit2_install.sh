...
# Check if ROS2 Humble or Foxy is installed.
if [ -f /opt/ros/foxy/setup.bash ]; then
    ...
    echo "This script must be run with ROS2 Humble or Foxy"
    read -p "Press any key to exit..."
    exit 1
fi
...
if [ "$ROS_DISTRO" = "foxy" ]; then
    ...
    # Install ros_control
    sudo apt-get install ros-foxy-controller-interface ros-foxy-controller-manager-msgs ros-foxy-controller-manager -y
elif [ "$ROS_DISTRO" = "humble" ]; then
    # Install moveit
    sudo apt-get install ros-humble-moveit -y
    # Install ros_control
    sudo apt-get install ros-humble-controller-interface ros-humble-controller-manager-msgs ros-humble-controller-manager -y
fi
